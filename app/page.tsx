...
type Picks = { user_id:string; final4:number[]; winner:number|null; weekly:Record<string,number> }
type Group = { code:string; name:string; created_by:string; member_ids:string[]; members:Record<string,{name:string;avatar:string;isAdmin:boolean}> }
...
const css = `
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .avatar{width:32px;height:32px;border-radius:50%;object-fit:cover;flex-shrink:0;background:var(--card2);display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700}
...
export default function App() {
  ...
  const [myPicks, setMyPicks] = useState<Picks>({ user_id:'', final4:[], winner:null, weekly:{} })
  ...
  useEffect(() => {
    if(!user) return
    fetch(`/api/picks?userId=${user.id}`).then(r=>r.json()).then(d => {
      if(d?.user_id) setMyPicks(d)
    })
  }, [user])
  ...
  useEffect(() => {
    if(!user) return
    fetch(`/api/groups?userId=${user.id}`).then(r=>r.json()).then(d => {
      if(Array.isArray(d)) setGroups(d)
    })
  }, [user])

  // Load all picks when viewing group
  useEffect(() => {
    if(!openGroup) return
    const ids = openGroup.member_ids.join(',')
    fetch(`/api/picks?userIds=${ids}`).then(r=>r.json()).then(d => {
      if(Array.isArray(d)) setAllPicks(d)
    })
  }, [openGroup])
  ...
  async function savePicks(updates: Partial<Picks>) {
    if(!user) return
    const merged = { ...myPicks, ...updates, user_id: user.id }
    setMyPicks(merged as Picks)
    await fetch('/api/picks', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ userId: user.id, ...updates }) })
  }

  async function saveAdmin(updates: Partial<Admin>) {
    const merged = { ...admin, ...updates }
    setAdmin(merged as Admin)
    await fetch('/api/admin', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(updates) })
  }

  async function createGroup() {
    if(!user || !newGroupName.trim()) return
    const res = await fetch('/api/groups', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ name: newGroupName.trim(), userId: user.id, userName: user.user_metadata?.full_name || 'You', userAvatar: user.user_metadata?.avatar_url || '' }) })
  ...
  async function joinGroup() {
    ...
    const res = await fetch('/api/groups', { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ code, userId: user.id, userName: user.user_metadata?.full_name || 'You', userAvatar: user.user_metadata?.avatar_url || '', action: 'join' }) })
    if(!res.ok) { showToast('Group not found', true); return }
    const g = await res.json()
    setGroups(gs => [...gs.filter(x=>x.code!==code), g])
    setJoinCode('')
    setOpenGroup(g)
  ...
  async function leaveGroup(code: string) {
    ...
    await fetch('/api/groups', { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ code, userId: user.id, action: 'leave' }) })
    setGroups(gs => gs.filter(g => g.code !== code))
    setOpenGroup(null)
    showToast('Left group')
  }

  async function deleteGroup(code: string) {
    if(!confirm('Delete group? This cannot be undone.')) return
  ...
  async function forceSync() {
    ...
    try {
      ...
      if(d.applied?.length) {
        ...
        setSyncMsg(d.message || 'No new results found')
      }
  ...
  // Render helpers
  const alive = Object.entries(admin.eliminated).length
    ? PAIRS.filter(p => !admin.eliminated[p.id])
    : PAIRS

  function Avatar({ uid, size=32 }: { uid: string, size?: number }) {
    const g = openGroup?.members[uid]
    const me = user?.id === uid
    const name = g?.name || (me ? user?.user_metadata?.full_name : '') || '?'
    const avatar = g?.avatar || (me ? user?.user_metadata?.avatar_url : '') || ''
    const color = COLORS[uid.charCodeAt(0) % COLORS.length]
    if(avatar) return <img src={avatar} style={{width:size,height:size,borderRadius:'50%',objectFit:'cover',flexShrink:0}} alt="" onError={e=>(e.currentTarget.style.display='none')} />
    return <div style={{width:size,height:size,borderRadius:'50%',background:color,display:'flex',alignItems:'center',justifyContent:'center',fontSize:size*0.4,fontWeight:700,color:'#fff',flexShrink:0}}>{name[0]?.toUpperCase()}</div>
  }

  function displayName(uid: string) {
    const g = openGroup?.members[uid]
    return g?.name || (uid === user?.id ? user?.user_metadata?.full_name : '') || 'Player'
  }
  ...
  if(!user) return <>
    ...
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',minHeight:'100vh',padding:24,textAlign:'center',gap:16}}>
      ...
      <div style={{fontSize:14,color:'var(--muted)',maxWidth:300}}>Season 35 Pick'em Challenge — sign in with Google to join or create a group</div>
      <button className="btn btn-gold" style={{marginTop:8,fontSize:15,padding:'12px 28px'}} onClick={signIn}>Sign in with Google</button>
    </div>
  </>

  const myName = user.user_metadata?.full_name || 'You'
  const myAvatar = user.user_metadata?.avatar_url || ''

  // ── PICKS TAB ──────────────────────────────────────────────────────
  function renderMyPicks() {
    const w = admin.current_week
    const locked = admin.picks_locked
    const actualElimThisWeek = Object.entries(admin.eliminated).find(([,wk]) => parseInt(String(wk)) === w)
    const actualElimId = actualElimThisWeek ? parseInt(actualElimThisWeek[0]) : null
    ...
    return <div className="view">
      ...
      {myPicksTab === 'preseason' && <>
        {!locked ? <>
          ...
          {PAIRS.map(p => {
            const inF4 = myPicks.final4?.includes(p.id)
            const full = (myPicks.final4?.length||0) >= 4 && !inF4
            return <div key={p.id} className={`popt ${inF4?'sel-f4':''}`} onClick={async()=>{
              if(full){showToast('Final 4 is full',true);return}
              const f4 = inF4 ? myPicks.final4.filter(x=>x!==p.id) : [...(myPicks.final4||[]),p.id]
          ...
          {(myPicks.final4?.length||0) >= 1 && <>
            ...
            {(myPicks.final4||[]).map(pairId => {
              const p = pairById(pairId)!
              const isW = myPicks.winner === pairId
          ...
          <div className="card"><div className="card-body">
            {myPicks.winner && (() => {
              const correct = admin.actual_winner ? myPicks.winner===admin.actual_winner : null
            ...
            {(myPicks.final4||[]).map(pid => {
              const correct = admin.actual_final4?.length > 0 ? admin.actual_final4.includes(pid) : null
      ...
      {myPicksTab === 'weekly' && <>
        ...
        {actualElimId && (() => {
          const ep = pairById(actualElimId)
          const correct = myPicks.weekly?.[w] === actualElimId
        ...
        {!actualElimId && <>
          ...
          {alive.map(p => {
            const isPicked = myPicks.weekly?.[w] === p.id
            const inMyF4 = myPicks.final4?.includes(p.id)
            const isMyWinner = myPicks.winner === p.id
            return <div key={p.id} className={`popt ${isPicked?'sel-elim':''}`} onClick={async()=>{
              const weekly = {...(myPicks.weekly||{}), [w]: p.id}
              await savePicks({weekly})
            }}>
      ...
      {Array.from({length:w},(_,i)=>w-i).map(wk => {
        ...
        return <div key={wk} className="card">
          ...
          {ep && <div className="card-body">
            {['Correct','Wrong','No pick'].map(type => {
              const filtered = allPicks.filter(p => {
                const pick = p.weekly?.[wk]
                if(type==='Correct') return pick === elimId
                if(type==='Wrong') return pick && pick !== elimId
                return !pick
              })
  ...
  function renderGroups() {
    if(openGroup) {
      ...
      const isAdmin = openGroup.members[user.id]?.isAdmin
      ...
      return <div className="view">
        ...
        {groupTab==='lb' && <div className="card"><div>
          {ranked.map((r,i) => <div key={r.uid} className={`lbrow${r.uid===user.id?' me-row':''}`}>
            <div style={{fontFamily:'serif',fontSize:20,fontWeight:900,color:i<3?'var(--gold)':'var(--muted)',minWidth:26}}>{i+1}</div>
            <Avatar uid={r.uid} />
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:13,fontWeight:600,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{displayName(r.uid)}{r.uid===user.id?' (you)':''}</div>
        ...
        {groupTab==='picks' && ranked.map(r => {
          const picks = picksMap[r.uid] || {}
          const wPick = picks.weekly?.[admin.current_week]
          return <div key={r.uid} className="card" style={{marginBottom:9}}>
            <div className="card-head" style={{padding:'10px 13px'}}>
              <div style={{display:'flex',alignItems:'center',gap:8}}><Avatar uid={r.uid} size={26} /><div style={{fontSize:13,fontWeight:600}}>{displayName(r.uid)}{r.uid===user.id?' (you)':''}</div></div>
            ...
            <div className="card-body" style={{padding:'10px 13px',fontSize:11,color:'var(--muted)',display:'grid',gap:4}}>
              ...
              <div>🔝 Final 4: {(picks.final4||[]).length?(picks.final4||[]).map(id=>pairById(id)?.celeb).join(', '):'None'}</div>
        ...
        {groupTab==='info' && <>
          ...
          <div style={{fontSize:12,color:'var(--muted)',marginBottom:6}}>Share this so friends can join</div>
          ...
          <div className="card"><div className="card-body">
            {members.map(uid => <div key={uid} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 0',borderBottom:'1px solid var(--border)'}}>
              <Avatar uid={uid} />
              <div style={{flex:1,fontSize:13,fontWeight:600}}>{displayName(uid)}{uid===user.id?' (you)':''}</div>
    ...
    return <div className="view">
      ...
      <div className="card"><div className="card-body" style={{display:'grid',gap:10}}>
        {[{label:'🏅 Season Winner',color:'var(--gold)',pts:PTS.winner,sub:'Locked before Week 1'},{label:'🔝 Final Four (each)',color:'var(--blue)',pts:PTS.f4,sub:'Locked before Week 1'},{label:'❌ Weekly Elimination',color:'var(--rose)',pts:PTS.elim,sub:'Per correct pick'}].map((s,i) => <div key={i} style={{display:'flex',alignItems:'center',gap:10,paddingTop:i?10:0,borderTop:i?'1px solid var(--border)':undefined}}>
          ...
          <div style={{fontSize:11,fontWeight:700,padding:'3px 9px',borderRadius:14,background:`rgba(${s.color==='var(--gold)'?'201,150,58':s.color==='var(--blue)'?'74,143,212':'196,84,120'},.14)`,color:s.color}}>{s.pts} pts</div>
        </div>)}
      </div></div>

      <div className="slabel">My Groups</div>
      {!groups.length && <div style={{textAlign:'center',padding:'20px 16px',color:'var(--muted)',fontSize:13}}>No groups yet — create one or enter a code</div>}
  ...
  function renderAdmin() {
    ...
    return <div className="view">
      <div className="banner" style={{background:'linear-gradient(135deg,#1A1408,var(--card))',border:'1px solid var(--gborder)'}}>
        ...
        <div style={{fontSize:12,color:'var(--muted)'}}>Auto-syncs every Tuesday night. Enter results manually if needed.</div>
      </div>

      <div className="card" style={{marginBottom:14}}>
        <div className="card-body">
          <div style={{fontSize:13,fontWeight:600,marginBottom:5}}>Auto-sync status</div>
          <div style={{fontSize:11,color:'var(--muted)',marginBottom:10}}>Runs automatically every Tuesday at 11pm ET via Vercel cron. Searches the web for elimination results and updates all leaderboards.</div>
      ...
      <div className="card" style={{marginBottom:14}}><div className="card-body">
        ...
        <div style={{fontSize:11,color:'var(--muted)',marginBottom:10}}>{admin.picks_locked?'Players cannot change Final Four or Winner picks.':'Lock before the show starts.'}</div>
      ...
      <div style={{fontSize:11,color:'var(--muted)',marginBottom:9}}>Tap to set. Locks Week {w} picks immediately.</div>
      {alive.map(p => {
        const isSel = thisElimId === p.id
        return <div key={p.id} className={`popt ${isSel?'sel-elim':''}`} onClick={async()=>{
          const newElim = {...elim}
          Object.keys(newElim).forEach(pid => { if(newElim[pid]===w) delete newElim[pid] })
          ...
          await saveAdmin(updates)
          showToast(isSel?'Undone':p.celeb+' eliminated!')
        }}>
      ...
      <div style={{fontSize:11,color:'var(--muted)',marginBottom:9}}>{(admin.actual_final4||[]).length}/4 — set to score preseason picks</div>
      {PAIRS.map(p => {
        const inF4 = (admin.actual_final4||[]).includes(p.id)
        return <div key={p.id} className={`popt ${inF4?'sel-f4':''}`} onClick={async()=>{
          const f4 = inF4 ? admin.actual_final4.filter(x=>x!==p.id) : [...(admin.actual_final4||[]),p.id]
          if(!inF4 && f4.length>4){showToast('Final 4 full',true);return}
          const winner = inF4 && admin.actual_winner===p.id ? null : admin.actual_winner
          await saveAdmin({actual_final4:f4,actual_winner:winner})
        }}>
          <div className="pcheck">{inF4?'✓':''}</div>
      ...
      {(admin.actual_final4?.length ? PAIRS.filter(p=>admin.actual_final4.includes(p.id)) : PAIRS).map(p => {
        const isWin = admin.actual_winner === p.id
      ...
      {Object.keys(elim).length > 0 && <>
        ...
        <div className="card"><div className="card-body">
          {Object.entries(elim).sort((a,b)=>a[1]-b[1]).map(([pid,wk]) => {
            const p = pairById(parseInt(pid))
            return <div key={pid} className="bkrow">
              ...
              <button className="btn btn-sm" style={{fontSize:10,padding:'4px 8px'}} onClick={async()=>{
                const newElim = {...elim}; delete newElim[parseInt(pid)]
                await saveAdmin({eliminated:newElim})
              }}>Undo</button>
            </div>
          })}
        </div></div>
      ...
      <div style={{fontSize:11,color:'var(--muted)',textAlign:'center',marginTop:20,cursor:'pointer'}} onClick={signOut}>Sign out</div>
    </div>
  }
  ...
  return <>
    ...
    <header style={{padding:'18px 18px 14px',textAlign:'center',borderBottom:'1px solid var(--border)',background:'linear-gradient(170deg,#0D1B2E,#060D18)',position:'relative'}}>
      <div style={{position:'absolute',top:14,right:14,display:'flex',alignItems:'center',gap:8}}>
        {myAvatar && <img src={myAvatar} style={{width:28,height:28,borderRadius:'50%'}} alt="" />}
        <span style={{fontSize:12,fontWeight:600,color:'var(--muted)'}}>{myName.split(' ')[0]}</span>
      </div>
